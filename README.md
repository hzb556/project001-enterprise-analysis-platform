...
```
≤50MB 文件（浏览器处理）                >50MB 文件（服务端，内部版）
┌─────────────────────┐          ┌──────────────────┐
│ Excel 读取 ── 本地   │          │ 上传 → 分析 → 删文件 │
│ 列识别   ── 本地     │    ←→   │ 仅保留汇总结果      │
│ 计算分析 ── 本地     │    API  │                    │
│ 图表渲染 ── 本地     │          │ 注册/登录/权限      │
└─────────────────────┘          └──────────────────┘
...
| 🔒 **本地处理** | ≤50MB 文件数据永不离开浏览器 |
| 🖥 **大文件处理** | >50MB 自动上传服务端处理（仅内部版，商用版提示拆分） |
| ☁️ **可选存云** | 分析完成后可选择保存报告到服务器 |
| 📥 **HTML 导出** | 下载完整交互式 HTML 报告 |
...
| 版本 | 文件上限 | 说明 |
|------|---------|------|
| **内部版** | 无上限 | >50MB 自动上传服务器处理 |
| **商用版** | 50MB | 切换 `config.js` 中 `enableLargeFileServer: false` |
